...
inline LibXR::Quaternion<double> AdjustedPackedCameraRotation(
    const std::array<float, 4>& rotation_wxyz)
{
  const char* mode = std::getenv("XR_TRACKER_CAMERA_ROTATION_MODE");
  if (mode == nullptr || mode[0] == '\0' || std::strcmp(mode, "default") == 0)
  {
    return PackedCameraRotation(rotation_wxyz);
  }
  if (std::strcmp(mode, "identity") == 0)
  {
    return LibXR::Quaternion<double>();
  }
  if (std::strcmp(mode, "xyzw") == 0)
  {
    return LibXR::Quaternion<double>(rotation_wxyz[3], rotation_wxyz[0],
                                     rotation_wxyz[1], rotation_wxyz[2]);
  }

  LibXR::Quaternion<double> q = PackedCameraRotation(rotation_wxyz);
  if (std::strcmp(mode, "normalize") == 0)
  {
    q.normalize();
    return q;
  }
  if (std::strcmp(mode, "conjugate") == 0)
  {
    return -q;
  }

  const Eigen::Matrix<double, 3, 1> eulr = q.ToEulerAngleZYX();
  double roll = eulr[0];
  double pitch = eulr[1];
  double yaw = eulr[2];
  if (std::strcmp(mode, "pitch_flip") == 0)
  {
    pitch = -pitch;
  }
  else if (std::strcmp(mode, "roll_flip") == 0)
  {
    roll = -roll;
  }
  else if (std::strcmp(mode, "roll_pitch_flip") == 0)
  {
    roll = -roll;
    pitch = -pitch;
  }
  else if (std::strcmp(mode, "yaw_only") == 0)
  {
    roll = 0.0;
    pitch = 0.0;
  }
  else if (std::strcmp(mode, "yaw_neg_pitch") == 0)
  {
    const Eigen::Quaternion<double> qyaw(
        Eigen::AngleAxisd(yaw, Eigen::Vector3d::UnitZ()));
    const Eigen::Quaternion<double> qpitch(
        Eigen::AngleAxisd(-pitch, Eigen::Vector3d::UnitY()));
    return LibXR::Quaternion<double>(qyaw * qpitch);
  }
  else if (std::strcmp(mode, "euler_rebuild") != 0)
  {
    return q;
  }
  return LibXR::Quaternion<double>(
      LibXR::EulerAngle<double>(roll, pitch, yaw).ToQuaternion());
}
...
inline double SpCameraPosePhaseAlpha()
{
  const char* env = std::getenv("XR_TRACKER_CAMERA_POSE_PHASE_ALPHA");
...
inline LibXR::Transform<double> InterpolateCameraPosePhase(
    const LibXR::Transform<double>& current,
    const LibXR::Transform<double>& previous, double alpha)
{
  alpha = std::clamp(alpha, -1.0, 1.0);
  const Eigen::Vector3d t_current(current.translation.x(), current.translation.y(),
                                  current.translation.z());
  const Eigen::Vector3d t_previous(previous.translation.x(), previous.translation.y(),
                                   previous.translation.z());
  LibXR::Quaternion<double> rotation(
      Eigen::Quaternion<double>(current.rotation)
          .slerp(alpha, Eigen::Quaternion<double>(previous.rotation)));
  rotation.normalize();
  return LibXR::Transform<double>(
      rotation, LibXR::Position<double>(t_current + alpha * (t_previous - t_current)));
}
...
{
  const LibXR::Transform<double> dynamic_pose(camera_rotation, camera_translation);
  const char* mode = std::getenv("XR_TRACKER_CAMERA_STATIC_MODE");
  if (mode != nullptr && std::strcmp(mode, "none") == 0)
  {
    return dynamic_pose;
  }
  if (mode != nullptr && std::strcmp(mode, "static_first") == 0)
  {
    return gimbal_to_camera_transform_static + dynamic_pose;
  }
  if (mode != nullptr && std::strcmp(mode, "static_only") == 0)
  {
    return gimbal_to_camera_transform_static;
  }
  return dynamic_pose + gimbal_to_camera_transform_static;
}
...
inline LibXR::Transform<double> ArmorTrackerCameraRotationToTrackerWorldPose(
    ...
    const LibXR::Transform<double>& gimbal_to_camera_transform_static)
{
  return ArmorTrackerCameraRotationToTrackerWorldPose(
      camera_rotation, LibXR::Position<double>(0.0, 0.0, 0.0),
      gimbal_to_camera_transform_static);
}

inline std::size_t SpCameraPoseDelayFrames()
{
  const char* env = std::getenv("XR_TRACKER_CAMERA_POSE_DELAY_FRAMES");
  if (env == nullptr || env[0] == '\0')
  {
    return 0;
  }
  ...
  return static_cast<std::size_t>(std::min<long>(value, 5));
}
...
{
  return {armor.points[3], armor.points[0], armor.points[1], armor.points[2]};
}
...
inline cv::Mat EigenRotationToCvMat(const Eigen::Matrix3d& rotation)
{
  ...
  for (int row = 0; row < 3; ++row)
  {
    ...
    {
      out.at<double>(row, col) = rotation(row, col);
    }
  }
  return out;
}
                               ...
                               const cv::Mat& tvec)
{
  ...
  if (projected.size() != image_points.size() || image_points.empty())
  {
...
inline bool SolveTranslationForRotation(
    const std::vector<cv::Point3f>& object_points,
    const std::vector<cv::Point2f>& normalized_points,
    const Eigen::Matrix3d& r_optical_object, Eigen::Vector3d& t_optical_object)
{
  ...
  Eigen::MatrixXd a =
      Eigen::MatrixXd::Zero(static_cast<long>(object_points.size() * 2U), 3);
  Eigen::VectorXd b =
      Eigen::VectorXd::Zero(static_cast<long>(object_points.size() * 2U));
  for (std::size_t i = 0; i < object_points.size(); ++i)
  {
    ...
    const Eigen::Vector3d rotated = r_optical_object * object_point;
    const double xn = static_cast<double>(normalized_points[i].x);
    const double yn = static_cast<double>(normalized_points[i].y);
    if (!(std::isfinite(xn) && std::isfinite(yn)))
    {
      return false;
    }

    const long row = static_cast<long>(i * 2U);
    a(row, 0) = 1.0;
    a(row, 2) = -xn;
    b(row) = xn * rotated.z() - rotated.x();
    a(row + 1, 1) = 1.0;
    a(row + 1, 2) = -yn;
    b(row + 1) = yn * rotated.z() - rotated.y();
  }

  t_optical_object = a.colPivHouseholderQr().solve(b);
  if (!t_optical_object.allFinite() || !(t_optical_object.z() > 1e-6))
  {
    return false;
  }
  ...
  {
    const Eigen::Vector3d object_point(
        static_cast<double>(object_point_cv.x),
        static_cast<double>(object_point_cv.y),
        static_cast<double>(object_point_cv.z));
...
{
  return std::clamp(ParseEnvDouble("XR_TRACKER_SP_PAIR_DZ_ALPHA", 0.35), 0.0, 1.0);
}
