...
class Solver
{
 public:
  ...
  /**
   * @brief Project a world point into image pixels.
   */
  bool ReprojectWorldPoint(const Eigen::Vector3d& xyz_in_world,
                           cv::Point2f& image_point) const
  {
    const Eigen::Vector3d camera_point =
        R_camera_to_body_.transpose() *
        (R_body_to_world_.transpose() * xyz_in_world - t_camera_to_body_);
    if (!std::isfinite(camera_point.z()) || camera_point.z() <= 1e-6)
    {
      return false;
    }
    ...
    std::vector<cv::Point2f> image_points;
    cv::projectPoints(camera_points, cv::Vec3d{0.0, 0.0, 0.0},
                      cv::Vec3d{0.0, 0.0, 0.0}, camera_matrix_,
...
class Target
{
 ...
 private:
  ...
  {
    const bool lock_outpost_center = UseOutpostHeightModel();
