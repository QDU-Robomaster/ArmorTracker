...
fs::path imu_path_for(const std::string& dataset)
{
  ...
  {
    env = std::getenv("XR_RAW_CORNER_IMU_NEW");
  }
  else if (dataset == "new_internal_5p9_lossless")
  {
    env = std::getenv("XR_RAW_CORNER_IMU_5P9");
  }
  else if (dataset == "old_internal")
  {
    env = std::getenv("XR_RAW_CORNER_IMU_OLD");
  }
...
std::vector<CaseSpec> read_cases(const fs::path& path)
{
  ...
  for (const auto& row : read_tsv(path))
  {
    ...
    spec.frame_convention =
        as_int(row, "frame_convention", as_int(row, "coordinate_mode", 1));
...
armor_tracker_xr::InputArmor make_input(const Row& row)
{
...
int main(int argc, char** argv)
{
  ...
  out << "dataset\timage_timestamp_us\tsource\tcenter_x\tcenter_y\tcenter_z\t"
         "yaw\tarmor_x\tarmor_y\tarmor_z\tarmor_yaw\tselected_face\txr_jumped\t"
  ...
  for (const CaseSpec& spec : read_cases(cases_path))
  {
    ...
    config.frame_convention = spec.frame_convention;
    config.camera_matrix = {spec.camera.fx, 0.0, spec.camera.cx,
                            0.0, spec.camera.fy, spec.camera.cy,
                            0.0, 0.0, 1.0};
    armor_tracker_xr::LockedTracker tracker(config);
    const auto imu = read_imu_quats(imu_path_for(spec.dataset));
    std::map<uint64_t, std::vector<armor_tracker_xr::InputArmor>> frames;
    ...
    for (const auto& [timestamp_us, inputs] : frames)
    {
      ...
      const armor_tracker_xr::Output output = tracker.Step(
      ...
      out << spec.dataset << '\t' << timestamp_us << "\txr_raw_corner_cabi_"
          << output.state << '\t' << output.center.x() << '\t'
          << output.center.y() << '\t' << output.center.z() << '\t'
          << output.yaw << '\t' << output.armor.x() << '\t'
