  ...
  if (pair_recenter_alpha > 0.0 && rt_.tracked_armors_num == ArmorsNum::NORMAL_4)
  {
                                          ...
                                          int face_index)
    {
      ...
      const double yaw0 = armor_tracker::UnwrapYawNear(
          match.measured_yaw - face_index * angle_step,
      ...
      return std::pair<Eigen::Vector3d, double>(
          Eigen::Vector3d(measured_xyz.x() -
                              radius * std::cos(match.measured_yaw),
                          measured_xyz.y() -
                              radius * std::sin(match.measured_yaw),
                          measured_xyz.z() - z_offset),
          yaw0);
    };

    const auto left_anchor =
        estimate_center(pair_match.left, pair_match.left_match, pair_match.left_face);
    const auto right_anchor =
        estimate_center(pair_match.right, pair_match.right_match, pair_match.right_face);
