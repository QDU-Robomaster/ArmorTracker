...
// SP-compatible target model and EKF update path. Kept as template
// definitions because ArmorTracker is parameterized by CameraInfo.
    ...
    const ArmorDetectorResults& armors_msg, bool force)
{
             ...
             int face_index)
  {
    ...
    const double measured_yaw =
        SpDetectorYawNear(observation.armor.pose.rotation, pred_yaw);
  ...
  for (std::size_t anchor_rank = 0; anchor_rank < observations.size(); ++anchor_rank)
  {
    ...
    for (int anchor_face = 0; anchor_face < armor_count; ++anchor_face)
    {
      const double anchor_yaw =
          SpDetectorYawNear(anchor.armor.pose.rotation, anchor_face * angle_step);
  ...
  for (int left_face = 0; left_face < armor_count; ++left_face)
  {
    for (int right_face = 0; right_face < armor_count; ++right_face)
    {
      ...
      const double left_measured_yaw =
          SpDetectorYawNear(left->armor.pose.rotation, left_ref_yaw);
      const double right_measured_yaw =
          SpDetectorYawNear(right->armor.pose.rotation, right_ref_yaw);
                                 ...
                                 int face_index)
      {
        ...
        return Eigen::Vector3d(observation.xyz.x() - radius * std::cos(angle),
                               observation.xyz.y() - radius * std::sin(angle),
                               observation.xyz.z() - z_offset);
      };
...
void ArmorTracker<CameraInfoV>::SpUpdatePair(const SpPairMatch& pair_match)
{
  ...
  if (pair_match.tracked_face == pair_match.left_face)
  {
  ...
  ekf_.measurement = Eigen::Vector4d(pair_match.tracked_armor.pose.translation.x(),
